...
if (extension_loaded('openssl')) {
    class AES
    {
        function kn($k, $n)
        {
            return openssl_encrypt($n, 'aes-128-ecb', $k, OPENSSL_RAW_DATA | OPENSSL_ZERO_PADDING);
        }
    }
}
...
elseif (extension_loaded('mcrypt')) {
    class AES
    {
        ...
        function kn($k, $n)
        {
            mcrypt_generic_init($this->td, $k, $k);
            return mcrypt_generic($this->td, $n);
        }
    }
}
*/
    ...
    class AES
    {
        ...
        function k($k)
        {
            ...
            $rk[5] = $rk[1] ^ $rk[4]; $rk[6] = $rk[2] ^ $rk[5]; $rk[7] = $rk[3] ^ $rk[6];
    
            $t = $rk[7];
            $rk[8] = $rk[4] ^ ($s[$t >> 24 & 0xff] | ($s[$t & 0xff] << 8) | ($s[$t >> 8 & 0xff] << 16) | (($s[$t >> 16 & 0xff] ^ 0x02) << 24));
            $rk[9] = $rk[5] ^ $rk[8]; $rk[10] = $rk[6] ^ $rk[9]; $rk[11] = $rk[7] ^ $rk[10];
    
            $t = $rk[11];
            $rk[12] = $rk[8] ^ ($s[$t >> 24 & 0xff] | ($s[$t & 0xff] << 8) | ($s[$t >> 8 & 0xff] << 16) | (($s[$t >> 16 & 0xff] ^ 0x04) << 24));
            $rk[13] = $rk[9] ^ $rk[12]; $rk[14] = $rk[10] ^ $rk[13]; $rk[15] = $rk[11] ^ $rk[14];
    
            $t = $rk[15];
            $rk[16] = $rk[12] ^ ($s[$t >> 24 & 0xff] | ($s[$t & 0xff] << 8) | ($s[$t >> 8 & 0xff] << 16) | (($s[$t >> 16 & 0xff] ^ 0x08) << 24));
            $rk[17] = $rk[13] ^ $rk[16]; $rk[18] = $rk[14] ^ $rk[17]; $rk[19] = $rk[15] ^ $rk[18];
    
            $t = $rk[19];
            $rk[20] = $rk[16] ^ ($s[$t >> 24 & 0xff] | ($s[$t & 0xff] << 8) | ($s[$t >> 8 & 0xff] << 16) | (($s[$t >> 16 & 0xff] ^ 0x10) << 24));
            $rk[21] = $rk[17] ^ $rk[20]; $rk[22] = $rk[18] ^ $rk[21]; $rk[23] = $rk[19] ^ $rk[22];
    
            $t = $rk[23];
            $rk[24] = $rk[20] ^ ($s[$t >> 24 & 0xff] | ($s[$t & 0xff] << 8) | ($s[$t >> 8 & 0xff] << 16) | (($s[$t >> 16 & 0xff] ^ 0x20) << 24));
            $rk[25] = $rk[21] ^ $rk[24]; $rk[26] = $rk[22] ^ $rk[25]; $rk[27] = $rk[23] ^ $rk[26];
    
            $t = $rk[27];
            $rk[28] = $rk[24] ^ ($s[$t >> 24 & 0xff] | ($s[$t & 0xff] << 8) | ($s[$t >> 8 & 0xff] << 16) | (($s[$t >> 16 & 0xff] ^ 0x40) << 24));
            $rk[29] = $rk[25] ^ $rk[28]; $rk[30] = $rk[26] ^ $rk[29]; $rk[31] = $rk[27] ^ $rk[30];
    
            $t = $rk[31];
            $rk[32] = $rk[28] ^ ($s[$t >> 24 & 0xff] | ($s[$t & 0xff] << 8) | ($s[$t >> 8 & 0xff] << 16) | (($s[$t >> 16 & 0xff] ^ 0x80) << 24));
            $rk[33] = $rk[29] ^ $rk[32]; $rk[34] = $rk[30] ^ $rk[33]; $rk[35] = $rk[31] ^ $rk[34];
    
            $t = $rk[35];
            $rk[36] = $rk[32] ^ ($s[$t >> 24 & 0xff] | ($s[$t & 0xff] << 8) | ($s[$t >> 8 & 0xff] << 16) | (($s[$t >> 16 & 0xff] ^ 0x1b) << 24));
            $rk[37] = $rk[33] ^ $rk[36]; $rk[38] = $rk[34] ^ $rk[37]; $rk[39] = $rk[35] ^ $rk[38];
    
            $t = $rk[39];
            $rk[40] = $rk[36] ^ ($s[$t >> 24 & 0xff] | ($s[$t & 0xff] << 8) | ($s[$t >> 8 & 0xff] << 16) | (($s[$t >> 16 & 0xff] ^ 0x36) << 24));
            $rk[41] = $rk[37] ^ $rk[40]; $rk[42] = $rk[38] ^ $rk[41]; $rk[43] = $rk[39] ^ $rk[42];
        ...
        function kn($k, $n)
        {
            ...
            list(,$rk0, $rk1, $rk2, $rk3) = unpack('N4', $k);
            list(, $x0,  $x1,  $x2,  $x3) = unpack('N4', $n);
            ...
            $y0 = $t0[$x0 >> 24 & 0xff] ^ $t1[$x1 >> 16 & 0xff] ^ $t2[$x2 >> 8 & 0xff] ^ $t3[$x3 & 0xff] ^ $rk4;
            $y1 = $t0[$x1 >> 24 & 0xff] ^ $t1[$x2 >> 16 & 0xff] ^ $t2[$x3 >> 8 & 0xff] ^ $t3[$x0 & 0xff] ^ $rk5;
            $y2 = $t0[$x2 >> 24 & 0xff] ^ $t1[$x3 >> 16 & 0xff] ^ $t2[$x0 >> 8 & 0xff] ^ $t3[$x1 & 0xff] ^ $rk6;
            $y3 = $t0[$x3 >> 24 & 0xff] ^ $t1[$x0 >> 16 & 0xff] ^ $t2[$x1 >> 8 & 0xff] ^ $t3[$x2 & 0xff] ^ $rk7;

            // r2
            $rk0 = $rk4 ^ ($s[$rk7 >> 24 & 0xff] | ($s[$rk7 & 0xff] << 8) | ($s[$rk7 >> 8 & 0xff] << 16) | (($s[$rk7 >> 16 & 0xff] ^ 0x02) << 24));
            $rk1 = $rk5 ^ $rk0; $rk2 = $rk6 ^ $rk1; $rk3 = $rk7 ^ $rk2;

            $x0 = $t0[$y0 >> 24 & 0xff] ^ $t1[$y1 >> 16 & 0xff] ^ $t2[$y2 >> 8 & 0xff] ^ $t3[$y3 & 0xff] ^ $rk0;
            $x1 = $t0[$y1 >> 24 & 0xff] ^ $t1[$y2 >> 16 & 0xff] ^ $t2[$y3 >> 8 & 0xff] ^ $t3[$y0 & 0xff] ^ $rk1;
            $x2 = $t0[$y2 >> 24 & 0xff] ^ $t1[$y3 >> 16 & 0xff] ^ $t2[$y0 >> 8 & 0xff] ^ $t3[$y1 & 0xff] ^ $rk2;
            $x3 = $t0[$y3 >> 24 & 0xff] ^ $t1[$y0 >> 16 & 0xff] ^ $t2[$y1 >> 8 & 0xff] ^ $t3[$y2 & 0xff] ^ $rk3;

            // r3
            $rk4 = $rk0 ^ ($s[$rk3 >> 24 & 0xff] | ($s[$rk3 & 0xff] << 8) | ($s[$rk3 >> 8 & 0xff] << 16) | (($s[$rk3 >> 16 & 0xff] ^ 0x04) << 24));
            $rk5 = $rk1 ^ $rk4; $rk6 = $rk2 ^ $rk5; $rk7 = $rk3 ^ $rk6;
            ...
            // r4
            $rk0 = $rk4 ^ ($s[$rk7 >> 24 & 0xff] | ($s[$rk7 & 0xff] << 8) | ($s[$rk7 >> 8 & 0xff] << 16) | (($s[$rk7 >> 16 & 0xff] ^ 0x08) << 24));
            $rk1 = $rk5 ^ $rk0; $rk2 = $rk6 ^ $rk1; $rk3 = $rk7 ^ $rk2;
            ...
            // r5
            $rk4 = $rk0 ^ ($s[$rk3 >> 24 & 0xff] | ($s[$rk3 & 0xff] << 8) | ($s[$rk3 >> 8 & 0xff] << 16) | (($s[$rk3 >> 16 & 0xff] ^ 0x10) << 24));
            $rk5 = $rk1 ^ $rk4; $rk6 = $rk2 ^ $rk5; $rk7 = $rk3 ^ $rk6;
            ...
            // r6
            $rk0 = $rk4 ^ ($s[$rk7 >> 24 & 0xff] | ($s[$rk7 & 0xff] << 8) | ($s[$rk7 >> 8 & 0xff] << 16) | (($s[$rk7 >> 16 & 0xff] ^ 0x20) << 24));
            $rk1 = $rk5 ^ $rk0; $rk2 = $rk6 ^ $rk1; $rk3 = $rk7 ^ $rk2;
            ...
            // r7
            $rk4 = $rk0 ^ ($s[$rk3 >> 24 & 0xff] | ($s[$rk3 & 0xff] << 8) | ($s[$rk3 >> 8 & 0xff] << 16) | (($s[$rk3 >> 16 & 0xff] ^ 0x40) << 24));
            $rk5 = $rk1 ^ $rk4; $rk6 = $rk2 ^ $rk5; $rk7 = $rk3 ^ $rk6;
            ...
            // r8
            $rk0 = $rk4 ^ ($s[$rk7 >> 24 & 0xff] | ($s[$rk7 & 0xff] << 8) | ($s[$rk7 >> 8 & 0xff] << 16) | (($s[$rk7 >> 16 & 0xff] ^ 0x80) << 24));
            $rk1 = $rk5 ^ $rk0; $rk2 = $rk6 ^ $rk1; $rk3 = $rk7 ^ $rk2;
            ...
            // r9
            $rk4 = $rk0 ^ ($s[$rk3 >> 24 & 0xff] | ($s[$rk3 & 0xff] << 8) | ($s[$rk3 >> 8 & 0xff] << 16) | (($s[$rk3 >> 16 & 0xff] ^ 0x1b) << 24));
            $rk5 = $rk1 ^ $rk4; $rk6 = $rk2 ^ $rk5; $rk7 = $rk3 ^ $rk6;
            ...
            // r10
            $rk0 = $rk4 ^ ($s[$rk7 >> 24 & 0xff] | ($s[$rk7 & 0xff] << 8) | ($s[$rk7 >> 8 & 0xff] << 16) | (($s[$rk7 >> 16 & 0xff] ^ 0x36) << 24));
            $rk1 = $rk5 ^ $rk0; $rk2 = $rk6 ^ $rk1; $rk3 = $rk7 ^ $rk2;
            ...
            return pack('N4',
                ($x0 & 0xff000000) ^ ($x1 & 0xff0000) ^ ($x2 & 0xff00) ^ ($x3 & 0xff) ^ $rk0,
                ($x1 & 0xff000000) ^ ($x2 & 0xff0000) ^ ($x3 & 0xff00) ^ ($x0 & 0xff) ^ $rk1,
                ($x2 & 0xff000000) ^ ($x3 & 0xff0000) ^ ($x0 & 0xff00) ^ ($x1 & 0xff) ^ $rk2,
                ($x3 & 0xff000000) ^ ($x0 & 0xff0000) ^ ($x1 & 0xff00) ^ ($x2 & 0xff) ^ $rk3
            );
        }
    }
}
